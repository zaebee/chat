...
class LinguisticValidator:
    ...
    def validate_past_tense(self, event_type: str) -> ValidationResult:
        ...
        for pattern, pattern_type in self.compiled_patterns:
            if pattern.match(normalized):
                confidence = self._calculate_pattern_confidence(pattern_type, normalized)
                return ValidationResult(
                    is_valid=True,
    ...
    def _calculate_pattern_confidence(self, pattern_type: str, word: str) -> float:
        ...
        if len(word) < 3:
            base_confidence *= 0.7  # Short words are less reliable
        elif len(word) > 15:
            base_confidence *= 0.8  # Very long words might be compound
    ...
    def validate_event_name_style(self, event_type: str) -> ValidationResult:
        ...
        if not re.match(r'^[a-z][a-z0-9_]*[a-z0-9]$', event_type):
            ...
            suggestions.append(re.sub(r'[A-Z]', lambda m: '_' + m.group().lower(), event_type).strip('_'))

        # Check length (reasonable bounds)
        if len(event_type) < 3:
            issues.append("Event type too short (minimum 3 characters)")
        elif len(event_type) > 50:
            issues.append("Event type too long (maximum 50 characters)")
