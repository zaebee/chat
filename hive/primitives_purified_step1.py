"""
Core System Primitives: The Foundation of the System Architecture

This module implements the four fundamental primitives from the System Architecture Specification:
- A (Aggregate): Structural organization and state management
- T (Transformation): Processing functions and data transformation
- C (Connector): Communication, sensing, and translation
- G (Generator): Generative actions and broadcasting

These primitives form the foundation of the system architecture.
"""
...
class HiveComponent(ABC):
    """Base class for all system components following the Legibility principle."""
...
class Aggregate(HiveComponent):
    ...
    They are the structural components that maintain the shape and integrity
    of the system.
    """
...
class Transformation(HiveComponent):
    ...
    They are the processors that transform data within the system.
    """
...
class Connector(HiveComponent):
    ...
    They are the communication infrastructure that enables coordination.
    """
    ...
    async def translate(
        ...
        try:
            ...
            if source_protocol == "websocket" and target_protocol == "event":
                # Convert WebSocket message to Event Protocol
                return {
            ...
            elif source_protocol == "event" and target_protocol == "websocket":
                # Convert Event Protocol to WebSocket message
                return {
...
class EventGenerator(HiveComponent):
    """
    G (Generator): Generative actions and broadcasting.

    Event Generators are the event generation pattern that creates new
    instances, broadcasts information, and triggers system-wide changes.
    They are the generative force that enables scaling and replication.
    """
    ...
    async def replicate(
        ...
        This is the replication mechanism for creating new specialized forms.
        """
    ...
    def get_status(self) -> Dict[str, Any]:
        return {
            "component": f"EventGenerator:{self.name}",
            "type": "G",
            "id": self.id,
            "event_type": self.event_type,
    ...
    async def health_check(self) -> bool:
        """Check if event generator is healthy."""
        # Event generators are healthy if they can generate new events
        try:
