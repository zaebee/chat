...
import type { TransformationComponent, TransformationResult } from './index'
...
export async function testSacredLambdaEngine(): Promise<void> {
  ...
  const lambdaEngine: TransformationComponent = await createTransformationComponent('data_transformer', {
  ...
  const result: TransformationResult = await lambdaEngine.transform(testData)
  ...
  const processResult: TransformationResult = await lambdaEngine.process(testData)
  ...
  for (const testCase of edgeCases) {
    const edgeResult: TransformationResult = await lambdaEngine.transform(testCase.input)
