...
const processCommand = (command: string) => {
  ...
  if (parts.length >= 2 && parts[0] === 'bee') {
    ...
    const validBeeTypes: OrganellaType[] = ['worker', 'scout', 'queen', 'guard'];
    if (validBeeTypes.includes(beeType as OrganellaType)) { // Validate beeType
      messages.value.push({
        id: generateUUID(),
        text: '',
        sender_id: 'system',
        ...
        timestamp: new Date().toISOString(),
        is_bot: true,
        bee_organella_type: beeType as OrganellaType, // Cast to OrganellaType after validation
